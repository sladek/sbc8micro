...
fn main() {
    ...
    let result = cpu.memory.load_program_from_file("test.obj", 0x600u16);
    let addr = 0x0010u16;
    let addr16 = 0x1234u16;
    ...
    cpu.memory.write_byte(addr16, value8);
    cpu.memory.write_byte_zero_page(z_addr, value8);
    cpu.memory
        .write_byte_zero_page(z_addr.wrapping_add(cpu.x), value8);
    cpu.memory
        .write_byte_zero_page(z_addr.wrapping_add(cpu.y), value8);
    cpu.p.set_carry(false);
    cpu.sp = 0xff;
    println!("Content of 0x1234={:02X}", cpu.memory.read_byte(0x1234));
    let program = vec![
        0xA9, 0x55, // LDA #$55
        0x6D, 0x34, 0x12, //
        0x00, // BRK
