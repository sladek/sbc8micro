...
//! 
//! This crate provides the functionality similar to iSBC202 controller
//! which is modified to support 2 floppy disks and 2 hard drives
//! This is partially compatible with the examples of original BIOS 
//! of MDS800 system so only minimal chages (new disk parameter block)
//! are necessary to the BIOS itself
use crate::disk::sssd8fd::Floppy;
use crate::disk::Sector;
use crate::disk::{DataDeletedData, Result};
...
impl DiskTypes {
    fn get_name(&self) -> String {
        match self {
           DiskTypes::Fdd(fdd) => { fdd.get_name() }
           DiskTypes::Hdd(hdd) => { hdd.get_name() }
        }
    }
    fn is_read_only(&self) -> bool {
        match self {
           DiskTypes::Fdd(fdd) => { fdd.is_read_only() }
           DiskTypes::Hdd(hdd) => { hdd.is_read_only() }
        }
    }
    fn seek_write_sector(&mut self, sector: Sector) -> Result<()>{
        match self {
           DiskTypes::Fdd(fdd) => { fdd.seek_write_sector(sector) }
           DiskTypes::Hdd(hdd) => { hdd.seek_write_sector(sector) }
        }
    }
}
...
pub struct FdHdC {
    ...
    name: Option<String>,
    disks: [Option<DiskTypes>; NUMBER_OF_DISKS],
    active_disk: u8,
    cpu_memory: Option<Rc<RefCell<memory::Memory>>>,
}

impl Default for FdHdC {
    fn default() -> Self {
        FdHdC {
            dstat: 0,
            dstat_address: BASE,
            ...
            name: Some("FdHdC".to_string()),
            disks: [const { None }; NUMBER_OF_DISKS],
            active_disk: 0,
            cpu_memory: None,
        }
    }
...
impl FdHdC {
    ...
    /// Sets base address of fdc (sbc202).
    ///
    /// Base address is data port for sbc202
    /// Base address + 1 is address of controll and status port of sbc202
    ...
    /// Get name of the disk controler.
    ///
    /// Gets name of the disk controler
    ...
    /// Assign floppy disk
    /// 
    /// Assignes floppy disk to bay 1, 2
    pub fn set_disk(&mut self, disk: DiskTypes, number: u8) -> Result<()> {
        if number <= NUMBER_OF_DISKS as u8 {
            let file_name = disk.get_name();
            for disk in self.disks.iter_mut() {
                if disk.is_none() {
                    break
                }
                let disk_name = disk.as_ref().unwrap().get_name();
                if disk_name == file_name {
                    return Err(ErrorIndicators::AddressError)
                }
            }
            //let dsk = disk.
            self.disks[number as usize] = Some(disk);
    ...
    /// Remove drive
    /// 
    /// Removes drive[number] from the controller
    pub fn remove_disk(&mut self, number: u8) {
            self.rtype = 0b000_0010;
            match number {
                0 => {
                    self.disks[0] = None;
    ...
    /// Set active disk
    /// 
    /// Sets active disk drive 0, 1, 2, 3
    pub fn set_active_disk(&mut self, number: u8) {
        self.active_disk = number;
    }
    /// Set interrupt
    /// 
    ...
    fn process_iopb(&mut self, iopb: &Iopb) -> Result<Option<Dma>> {
        ...
        let discette_instruction = DiskInstruction::new(iopb.diskette_instruction);
        ...
        match opcode {
            ...
            opcode if opcode == Opcode::Seek as u8 => {
                ...
                match &mut self.disks[unit as usize] {
                    Some(disk) => {
                        match disk {
                            DiskTypes::Fdd(fdd) => { fdd.seek(track)?}
                            DiskTypes::Hdd(hdd) => { hdd.seek(track)?}
                        }
                    }
                    None => {
            ...
            opcode if opcode == Opcode::FormatTrack as u8 => {
                ...
                match &mut self.disks[unit as usize] {
                    Some(disk) => {
                        if disk.is_read_only() { return Err(ErrorIndicators::WriteProtect); }
                        match disk {
                            DiskTypes::Fdd(fdd) => { 
                                fdd.format_track(track)?
                            }
                            DiskTypes::Hdd(hdd) => { 
                                hdd.format_track(track)?;
                            }
                        };
                    }
                    None => {
            ...
            opcode if opcode == Opcode::Recalibrate as u8 => {
                match &mut self.disks[unit as usize] {
                    Some(disk) => {
                        match disk {
                            DiskTypes::Fdd(fdd) => { fdd.seek(0)?}
                            DiskTypes::Hdd(hdd) => { hdd.seek(0)?}                            
                        }
                    }
                    None => {
                        return Err(ErrorIndicators::SeekError)
    ...
    /// Read data fromd disk
    /// 
    /// Reads data fromdisk based on IOPB.
    fn read_data(&mut self, iopb: &Iopb) -> Result<Vec<u8>> {
        let discette_instruction = DiskInstruction::new(iopb.diskette_instruction);
        let unit = discette_instruction.get_unit_select();
            match &self.disks[unit as usize] {
                Some(disk) => {
                    ...
                    while sectors_to_read != 0 {
                        let sector = match disk {
                            DiskTypes::Fdd(fdd) => { fdd.read_sector(iopb.track_address, sector_num)?}
                            DiskTypes::Hdd(hdd) => { hdd.read_sector(iopb.track_address, sector_num)?}                            
                        };
    ...
    /// Write data todisk
    /// 
    /// Write data todisk based on IOPB.
...
impl IoPort for FdHdC {
    ...
    fn get_io_port_info(&self) -> String {
        ...
        let mut char_idx = 0;
        let disk_char = 'A';
        let mut disk_drives = String::new();
        for disk in disks {
            match disk {
                Some(disk) => {
                    let name = disk.get_name();
                    let is_ro = disk.is_read_only();
                    let disk_char = char::from_u32(disk_char as u32 + char_idx).unwrap();
                    let mut assigned_drive = format!(" {disk_char}:{name}");
                    char_idx += 1;
                    if is_ro {
                        assigned_drive.push_str("[RO]");
                    }
                    disk_drives.push_str(&assigned_drive);
                }
                None => {
                    break
                }
        ...
        format!(
            "Disk controller (2 x single sided single density 8 inch, 2 x hard disk 8MB): base address[{base_address}], name[{name}], drives:{disk_drives}",
        )
    }
...
mod tests {
    ...
    use crate::disk::hdd8m::Hdd;
    use crate::disk::{Sector, sssd8fd::Floppy};
    use crate::disk::{hdd8m, DataDeletedData};
    use crate::io::IoPort;
    use crate::io::fdhdc::{DiskTypes, FdHdC};
    ...
    fn test_io_mapped() {
        ...
        let mut fdhdc = FdHdC::new(cpu.get_memory_ref()); // Base address 0x78
        fdhdc.set_base_address(0x78);
        println!("Name: {:?}", fdhdc.get_name());
        println!("{}", fdhdc.get_io_port_info());
        let mut io_memory = IoMemory::new();
        let res = io_memory.map_port(Box::new(fdhdc));
        assert_eq!(res, Ok(()));
    }
    #[test]
    fn test_memory_mapped() {
        ...
        let _ = memory.borrow_mut().write_byte(0x55aa, 0x55);
        let mut fdhdc = FdHdC::new(cpu.get_memory_ref()); // Base address 0x78
        fdhdc.set_memory_base_address(0x1000);
        println!("Name: {:?}", fdhdc.get_name());
        println!("{}", fdhdc.get_io_port_info());
        let res = memory.borrow_mut().map_port(Box::new(fdhdc));
        assert_eq!(res, Ok(()));
    }
    #[test]
    fn test_iopb_read_floppy_sector() {
        let file_name = "disks/iopb_test_read_floppy_sector.dsk";
        ...
        let _ = fdhdc.set_disk(DiskTypes::Fdd(floppy), 0);
        fdhdc.set_base_address(0x78);
    ...
    #[test]
    fn test_iopb_read_hdd_sector() {
        let file_name = "disks/iopb_test_read_hdd_sector.dsk";
        init_hard_disk(file_name);
        // let's use that freshly created disk image and write some data
        let hdd = Hdd::new(file_name, false).unwrap();
        let mut hddt = DiskTypes::Hdd(hdd);
        let mut data = [0; 128];
        for i in 0..data.len() {
            data[i] = i as u8;
        };
        // Let's write one sector
        let sector = Sector::new(0, 1, &data);
        _ = hddt.seek_write_sector(sector);
        let mut cpu = i8080::Cpu::new();
        let iopb_address = 0x2000;
        let program_address = 0x1000;
        let ilow = 0x79u8;
        let ihigh = 0x7au8;
        let mut fdhdc = Box::new(FdHdC::new(cpu.get_memory_ref())); // Base address 0x78
        // Let's assign the disk as disk[0] to the controller
        let _ = fdhdc.set_disk(hddt, 0);
        fdhdc.set_base_address(0x78);
        let io_memory = cpu.get_io_memory().unwrap();
        let res = io_memory.map_port(fdhdc);
        assert_eq!(Ok(()), res);
    ...
    fn test_iopb_read_multiple_floppy_sectors() {
        ...
        let ilow = 0x79u8;
        let ihigh = 0x7au8;
        let mut fdc = Box::new(FdHdC::new(cpu.get_memory_ref())); // Base address 0x78
        // Let's assign the floppy as floppy[0] to the controller
        let _ = fdc.set_disk(DiskTypes::Fdd(floppy), 0);
    ...
    fn test_iopb_read_multiple_hdd_sectors() {
        ...
        // Let's write two sector
        let sector = Sector::new(0, 1, &data);
        _ = hdd.seek_write_sector(sector);
        let sector = Sector::new(0, 2, &data);
        _ = hdd.seek_write_sector(sector);
        let mut cpu = i8080::Cpu::new();
        let iopb_address = 0x2000;
        ...
        let mut fdc = Box::new(FdHdC::new(cpu.get_memory_ref())); // Base address 0x78
        // Let's assign the floppy as floppy[0] to the controller
        let _ = fdc.set_disk(DiskTypes::Hdd(hdd), 0);
        fdc.set_base_address(0x78);
        let io_memory = cpu.get_io_memory().unwrap();
        let res = io_memory.map_port(fdc);
        assert_eq!(Ok(()), res);
    ...
    fn test_iopb_read_hdd_multiple_sectors_error() {
        ...
        // Let's write two sector
        let sector = Sector::new(0, sector_num, &data);
        let res1 = hdd.seek_write_sector(sector);
        println!("{:?}", res1);
        let sector = Sector::new(0, sector_num + 1, &data);
        let res2 = hdd.seek_write_sector(sector);
        println!("{:?}", res2);
        let mut cpu = i8080::Cpu::new();
        let iopb_address = 0x2000;
        let program_address = 0x1000;
    ...
    #[test]
    fn test_iopb_floppy_verify_crc() {
    ...
    #[test]
    fn test_iopb_floppy_recalibrate() {
    ...
    #[test]
    fn test_iopb_floppy_format_track() {
        let file_name = "disks/iopb_test.dsk";
        init_floppy_disk(file_name);
        // let's use that freshly created disk image and write some data
        let mut floppy = Floppy::new(file_name, false).unwrap();
    ...
    #[test]
    fn test_iopb_floppy_read_sector_mem_mapped() {
        ...
        // Let's assign the floppy as floppy[0] to the controller
        let _ = fdhdc.set_disk(DiskTypes::Fdd(floppy), 0);
        fdhdc.set_memory_base_address(base_addr);
        let res = cpu.get_memory().map_port(fdhdc); 
        assert_eq!(Ok(()), res);
    ...
    #[test]
    fn test_iopb_floppy_read_sector_mem_mapped_6502() {
    ...
    #[test]
    fn test_iopb_floppy_write_sector() {
    ...
    #[test]
    fn test_iopb_floppy_write_2_sectors() {
    ...
    #[test]
    fn test_iopb_floppy_write_2_sectors_error() {
    ...
    fn test_iopb_hdd_write_2_sectors_error() {
        ...
        let iopb: &[u8] = &[
            ...
            0x02, // Number of records  
            0x00, // Track address
            Hdd::NUM_OF_SECTORS_PER_TRACK, // Sector address 
            0x00, // buffer address Lower. 0x3000 buffer address for read data from fdc
            0x30, // buffer address Upper.
        ];
        let program: &[u8] = &[
    ...
    #[test]
    fn test_iopb_floppy_write_sector_deleted_data() {
