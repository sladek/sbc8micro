...
impl Async8251 {
    ...
    ///
    /// Reads data from serial port and sets status flags accordingly.
    ...
    fn read_serial_to_rx_buffer(&mut self) {
        if let Some(port) = self.port.as_mut() {
            ...
            if let Ok(t) = port.read(&mut buffer) {
                self.data.rx_data = buffer[0];
                self.status_control.status |= Status::RxRDY as u8; // Set RxRDY
                let mut i = 1; // The first is already in the rx_data
                while i < t {
                    if self.data.rx_buffer.push(buffer[i]).is_err() {
    ...
    pub fn open_port(mut self, port_name: &str) -> Result<Self, serialport::Error> {
        ...
        match serialport::new(port_name, self.get_baud_rate())
            .data_bits(self.get_data_bits())
            .stop_bits(stop_bits)
            //            .flow_control(flow_control) // Flow control has not been implemented yet
            .parity(parity)
...
#[cfg(test)]
mod tests {
    use crate::io::serial::{Async8251, BaudRateFactor, CharacterLength, FiFo, State, StopBits};
